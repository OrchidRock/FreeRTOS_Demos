...
#define ESP8266_PB_RST   GPIO_PIN_5 // PB5
// #define ESP8266_PB_RST   GPIO_PIN_1  // PB1

#define ESP8266_UART     1   // UART1: PB1/PB0
// #define ESP8266_UART     2   // UART2: PD7/PD6
...
bool ReceiveDataFilter(char letter){
  if (ReceiveDataState > 4 || ReceiveDataState < 1) ReceiveDataState = 1;
...
void ESP8266_OutChar(char data){
    ...
    if(ESP8266_EchoCommand) UARToutChar(data); // echo debugging
}
...
void static ESP8266RxToBuffer(void){
    ...
    while(((UART_ESP8266(_FR_R) & UART_FR_RXFE) == 0) && (ESP8266RxFifo_Size() < (FIFOSIZE - 1))){
        letter = UART_ESP8266(_InChar)();
        if(ESP8266_EchoResponse){
            UARToutCharNonBlock(letter); // echo
        }
        if (letter == '>') {
            LED_BlueOff();
        }
...
void UART_ESP8266(_Handler)(void){
    if(UART_ESP8266(_IntStatus)() & UART_RIS_RXRIS){       // hardware RX FIFO >= 2 items
        UART_ESP8266(_IntClear)(UART_ICR_RXIC);        // acknowledge RX FIFO
        ESP8266RxToBuffer();
    }
    if(UART_ESP8266(_IntStatus)() & UART_RIS_RTRIS){       // receiver timed out
        UART_ESP8266(_IntClear)(UART_ICR_RTIC);        // acknowledge receiver time out
        ESP8266RxToBuffer();
    }
}
...
int ESP8266_Init(int rx_echo, int tx_echo){ char c; const char* s; uint32_t timer = 1;
  ...
  ROM_GPIOPinTypeGPIOOutput(GPIO_PORTB_BASE, ESP8266_PB_RST);

  // Hard reset
  GPIOPinWrite(GPIO_PORTB_BASE, ESP8266_PB_RST, 0);
  systick_wait_ms(10);
  GPIOPinWrite(GPIO_PORTB_BASE, ESP8266_PB_RST, ESP8266_PB_RST);
  
  // Wait for ready status with timeout
  // Use low-level UART communication, interrupts disabled
  s = ESP8266_READY_RESPONSE;
  
  timer = 50000000;  // around a 1-2s total timeout
  
  while(*s) {
    while(timer && ((UART_ESP8266(_FR_R) & UART_FR_RXFE) != 0)) { timer--; }
    if(!timer) break;  
    c = (char)UART_ESP8266(_InChar)();
    if(rx_echo) UARToutCharNonBlock(c); // echo, requires UART0 to be operational, will print garbage
    if(c == *s) {
      s++;
    } else {
      s = ESP8266_READY_RESPONSE;
